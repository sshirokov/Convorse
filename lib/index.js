...
Convorse.prototype.generate_endpoint = function(dotpath, url, method, params) {
    var self = this;
    function url_modify_with(args) {
        return function() {
            if(arguments.length != args.length)
                throw "Wrong number of arguments! (" + arguments.length + " instad of " + args.length + ")";
            args.forEach(function(arg, i) { url = url.replace(':'+arg, arguments[i]); });
            console.log("Hitting:", method, "=>", url);
            consile.log("Params:", params);
        };
    }
    dotpath.reduce(
        function(cursor, i) {
